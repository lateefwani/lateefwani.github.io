...
date: 2021-01-04 00:00:00 +05:30
...
base_dir <- here::here()
base_url <- '/'
fig_path <- 'images/posts/gps-gis-osm/'

knitr::opts_knit$set(base.dir = base_dir, base.url = base_url, global.par = T)
...
a handful of designated sites.`[^dispersed]`{=markdown}

`[^dispersed]: `{=markdown} See [here](https://sectionhiker.com/what-is-the-difference-between-frontcountry-camping-backcountry-or-designated-campsites-and-dispersed-camping/) for a discussion of different types of campsites and contexts in which they are usually found.
...
![](/images/posts/gps-gis-osm/ut_query.png){.align-center}
...
![](/images/posts/gps-gis-osm/ut_select.png){.align-center}
...
![](/images/posts/gps-gis-osm/ut_rel.png){.align-center}
...
![](/images/posts/gps-gis-osm/unf.png){.align-center}
...
grab both rivers and streams in the area.`[^opq-multiple]`{=markdown}

`[^opq-multiple]: `{=markdown} If we didn't do this, we'd have to use `c()` to combine multiple `osmdata_sf` objects and then extract the `osm_lines` object from the combined `osmdata_sf` object.
...
refers to forest roads.`[^forest-roads]`{=markdown} OSM identifies roads with
...
`[^forest-roads]: `{=markdown} The US Forest Service maintains GIS data on forest roads on National Forest land, but the [API](https://data-usfs.hub.arcgis.com/datasets/national-forest-system-roads-feature-layer) to access them is...less than user friendly so I'm ignoring them for this illustration.
...
disturbance to the surrounding area.`[^disturbance]`{=markdown} All of the
...
```{r campsites_animation, echo = F, animation.hook = 'gifski'}
## no margins; reset b/c gifski doesn't seem to inherit knitr global pars
#par(mar = c(0, 0, 0, 0))

## buffer water 750 ft
campsites_plot_orig <- st_union(ut_water) %>% 
  st_buffer(dist = units::set_units(750, ft))

## plot using bbox of original plot to maintain image position + scale
plot(campsites_plot_orig, col = 'lightgreen', border = NA,
     xlim = st_bbox(campsites_plot_orig)[c(1, 3)],
     ylim = st_bbox(campsites_plot_orig)[c(2, 4)])

## buffer water 200 ft and subtract
campsites_plot <- st_union(ut_water) %>% 
  st_buffer(dist = units::set_units(200, ft)) %>% 
  st_difference(x = campsites_plot_orig)

## plot using bbox of original plot to maintain image position + scale
plot(campsites_plot, col = 'lightgreen', border = NA,
     xlim = st_bbox(campsites_plot_orig)[c(1, 3)],
     ylim = st_bbox(campsites_plot_orig)[c(2, 4)])

## buffer trail 1/4 mile and intersect
campsites_plot <- st_union(ut) %>% 
  st_buffer(dist = units::set_units(.25, mi)) %>% 
  st_intersection(x = campsites_plot)
...
campsites_plot <- st_union(ut) %>% 
  st_buffer(dist = units::set_units(200, ft)) %>% 
  st_difference(x = campsites_plot)
...
## buffer roads to 1000 ft and subtract
campsites_plot <- st_union(ut_roads) %>% 
  st_buffer(dist = units::set_units(1000, ft)) %>% 
  st_difference(x = campsites_plot)
...
## cast multipolygon to polygons and convert to sf
campsites_plot <- campsites_plot %>% 
  st_cast('POLYGON') %>% 
  st_sf() %>% 
  mutate(id = 1:n()) %>%
  filter(st_area(.) > units::set_units(100000, m^2)) # filter to > .1 sq km

## plot using bbox of original plot to maintain image position + scale
plot(campsites_plot$geometry, col = 'lightgreen', border = NA,
     xlim = st_bbox(campsites_plot_orig)[c(1, 3)],
     ylim = st_bbox(campsites_plot_orig)[c(2, 4)])
```
...
```{r plot_pars_raster, include = F}
par(mar = c(5, 4, 4, 2) + 0.1)
```

```{r elevation, dependson = 'campsites'}
library(raster)
library(elevatr)
...
```{r slope_subset, dependson = 'slope'}
...
```{r plot_final, dependson = 'slope_subset'}
...
then save them to disk to import into CalTopo.`[^export]`{=markdown}

`[^export]: `{=markdown} Want to find potential campsites for a trail that's not in OpenStreetMap? CalTopo supports exports as well as imports, so you can trace the route in CalTopo, export it, then load it in R with `sf::st_read()` and then carry out the steps above!
...
GeoJSON.`[^json-field]`{=markdown}

`[^json-field]: `{=markdown} CalTopo refers to an object's name field as its "Label" in the interface, but this isn't what it's called under the hood. I had to export a line I create and inspect the resulting `.json` file to find out that it's referred to as a "title" instead.

```{r geojson_export, results = 'hide', dependson = 'slope_subset'}
library(geojsonio)
...
<iframe src="https://caltopo.com/m/HGMR" height="600px" width="100%" style="border:none;"></iframe>
